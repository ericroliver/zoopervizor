...
export async function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    ConfigurationManager.onConfigChange((newConfig) => {
      handleConfigChange(newConfig).catch((error) => {
...
function setupEventForwarding(): void {
  ...
  allEvents.forEach(eventName => {
    listener.onEvent(eventName, (...args: unknown[]) => {
...
async function handleConfigChange(newConfig: ZoopervizorConfig): Promise<void> {
  ...
  if (newConfig.api.enabled && (!apiServer?.isRunning())) {
  ...
  } else if (!newConfig.api.enabled && apiServer?.isRunning()) {
