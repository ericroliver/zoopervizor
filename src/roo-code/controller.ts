...
export class RooCodeController {
	...
	async createProfile(name: string, profile?: any, activate?: boolean): Promise<string> {
		if (!this.api) {
			throw new Error('Roo-Code API not connected');
		}
	...
	async updateProfile(name: string, profile: any, activate?: boolean): Promise<string | undefined> {
		if (!this.api) {
			throw new Error('Roo-Code API not connected');
		}
